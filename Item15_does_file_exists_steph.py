...
# Item 15               #### sjb bug fix  Case# 3
def does_file_exist(user_input):
    """ Grab user input from student_data_manager() to check for 
    the existance of file requested, using os.path.exists(user_input). 
     
    try: if file is valid return True
    except FileNotFound Error: Invalid Entry. Please try again. 
    """
...
def student_data_manager(user_input):
    ...
    if user_input in ["0", "1", "2", "3", "4", "5", "6"]:
        ...
        #  if entry is 2: Update an existing File            ##### sjb bug fix Case# 3
        elif user_input == 2:
            file_to_update = input("Enter the file name to update (or press enter to cancel): ")
            
            if file_to_update != "":
                if does_file_exist(file_to_update) == True:
